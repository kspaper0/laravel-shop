    ...
    <div>
      <form class="form-horizontal" role="form" id="order-form">
        <div class="form-group">
          <label class="control-label col-sm-4">Please choose postal address</label>
        ...
        <!-- 优惠码开始 -->
        <div class="form-group">
          <label class="control-label col-sm-4">Coupon Code</label>
          <div class="col-sm-4">
...
<script>
  $(document).ready(function () {
    ...
    $('.btn-remove').click(function () {
      ...
      swal({
        title: "Are you sure to remove the item from Shopping Cart? ",
        icon: "warning",
    ...
    $('.btn-create-order').click(function () {
      ...
      // 遍历 <table> 标签内所有带有 data-id 属性的 <tr> 标签
      // 也就是每一个购物车中的商品 SKU
    ...
    $('#btn-check-coupon').click(function () {
      ...
      // 如果没有输入则弹框提示
      if(!code) {
        swal('Please type in coupon code', '', 'warning');
        return;
      }
      // 调用检查接口
      axios.get('/coupon_codes/' + encodeURIComponent(code))
        ...
        // then 方法的第一个参数是回调，请求成功时会被调用
          ...
          if(error.response.status === 404) {
            swal('The coupon code is not existed', '', 'error');
