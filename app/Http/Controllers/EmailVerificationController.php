...
class EmailVerificationController extends Controller
{
    public function verify(Request $request)
    {
        ...
        if ($token != Cache::get('email_verification_'.$email)) {
            throw new InvalidRequestException('Verification link does not exist or has expired');
        }
        ...
        // 最后告知用户邮箱验证成功。
        return view('pages.success', ['msg' => 'Successfully']);
    }
    ...
    public function send(Request $request)
    {
        ...
        if ($user->email_verified) {
            throw new InvalidRequestException('Your Email has been actived');
        }
        // 调用 notify() 方法用来发送我们定义好的通知类
        $user->notify(new EmailVerificationNotification());

        return view('pages.success', ['msg' => 'Email has been sent']);
    }
}