...
class OrdersController extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->total_amount('Total Cost')->sortable();
    ...
    public function ship(Order $order, Request $request)
    {
        ...
        if ($order->ship_status !== Order::SHIP_STATUS_PENDING) {
            throw new InvalidRequestException('The order has delivered');
        }
        if($order->type === Order::TYPE_CROWDFUNDING && $order->items[0]->product->crowdfunding->status !== CrowdfundingProduct::STATUS_SUCCESS) {
            throw new InvalidRequestException('The good has still been raising funds');
        }
    ...
    public function handleRefund(Order $order, HandleRefundRequest $request)
    {
        ...
        if ($order->refund_status !== Order::REFUND_STATUS_APPLIED) {
            throw new InvalidRequestException('The order has not applied refund');
        }
        // 是否同意退款
